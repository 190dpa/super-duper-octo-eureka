...
async function updateMainEmbed() {
  ...
  try {
    ...
    // Se mainMessageId existir, edita a mensagem. Senão, envia uma nova.
    const message = await sendMessageWithBot(CONFIG.mainChannelId, CONFIG.mainMessageId || null, body);
...
async function startServer() {
  ...
  bot.on('interactionCreate', async interaction => {
    try {
      ...
      if (interaction.isButton() && interaction.customId === 'buy_item_button') {
        ...
        if (availableStock.length === 0) {
          await interaction.reply({ content: 'Desculpe, todos os nossos itens estão esgotados no momento.', ephemeral: true });
          return;
        }
        ...
        await interaction.reply({
          ...
          ephemeral: true // Apenas o usuário que clicou verá esta mensagem
        });
      }

      // --- Manipulador da Seleção do Item ---
      if (interaction.isStringSelectMenu() && interaction.customId === 'select_item_to_buy') {
        ...
        if (!item) {
          await interaction.update({ content: 'O item selecionado não foi encontrado.', components: [], ephemeral: true });
          return;
        }
        ...
        await thread.send(`Olá <@${interaction.user.id}> e <@${owner.id}>! Este é o seu ticket para a compra de **${item.emoji} ${item.name}**.\nPor favor, discutam os detalhes da transação aqui.`);
        ...
        await interaction.update({ content: `Seu ticket de compra foi criado com sucesso: <#${thread.id}>`, components: [], ephemeral: true });
      }

      // --- Manipulador do Botão "Confirmar Entrega" ---
      if (interaction.isButton() && interaction.customId.startsWith('confirm_delivery_')) {
        const owner = (await bot.application.fetch()).owner;
        ...
        if (!item) {
          return interaction.reply({ content: 'Erro: Item não encontrado.', ephemeral: true });
        }
